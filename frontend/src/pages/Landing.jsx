...
import { ArrowRight, ChevronRight, Crown, Users, Puzzle, Bot, MessageSquare } from 'lucide-react';
...
export default function Landing({ onSelectMode }) {
  ...
  const modes = [
    ...
    },
    {
      id: 'online',
      title: 'Play Online',
      desc: 'Compete with real players around the world.',
    ...
    },
    {
      id: 'puzzle',
      title: 'Puzzle Mode',
      desc: 'Solve chess puzzles and train your brain.',
  ...
  return (
    <div className="relative min-h-[calc(100vh-80px)] flex flex-col justify-between max-w-7xl mx-auto px-6 pt-4 pb-12 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col justify-center space-y-10 z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 items-center gap-6 pt-2">
          ...
          <div className="lg:col-span-6 flex justify-center lg:justify-end relative">
            <div className="relative w-full max-w-[420px] aspect-square rounded-2xl overflow-hidden flex items-center justify-center group">
              {/* Image with subtle fade-to-background edge overlay */}
              <img
                src="/chess_knight_hero.jpg"
                alt="ChessCure 3D Golden Rim-lit Knight"
                className="w-full h-full object-cover object-center transform transition-transform duration-700 group-hover:scale-105"
              />
              {/* Smooth gradient blend into the dark canvas at bottom/sides */}
        ...
        {/* Challenge Banner */}
        <div 
          onClick={() => onSelectMode && onSelectMode('challenge')}
          className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-[#0b1424] via-[#0e192c] to-[#0c1525] border border-[#1b283f] px-6 py-5 flex items-center justify-between cursor-pointer hover:border-slate-600 transition-all duration-300 shadow-xl group"
        >
          ...
          <div className="flex items-center gap-5 z-10">
            ...
            <div>
              ...
              <p className="text-slate-400 text-xs sm:text-sm mt-0.5">
                and unlock special chats
              </p>
            </div>
          </div>

          {/* Right section: Watermark chat bubbles + chevron */}
          <div className="flex items-center gap-4 z-10">
            {/* Subtle background chat bubbles watermark */}
            <div className="hidden sm:flex items-center gap-1.5 opacity-20 text-slate-300 group-hover:opacity-30 transition-opacity mr-2">
              <div className="relative">
                <MessageSquare className="w-10 h-10 fill-current" />
                <MessageSquare className="w-6 h-6 fill-current absolute -bottom-2 -left-2 text-slate-400" />
              </div>
            </div>
      ...
      {/* Footer / Quote Section with subtle checkered corner pattern */}
